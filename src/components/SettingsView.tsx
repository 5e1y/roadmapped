  ...
  return (
    <section className="flex flex-col gap-m">
      <div>
        ...
        {hint && <p className="mt-xs text-xs text-textsoft">{hint}</p>}
      </div>
      {children}
    </section>
  )
  ...
  return (
    <ViewShell>
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto flex max-w-2xl flex-col gap-[calc(var(--spacing-xl)_+_var(--spacing-s))] px-xl py-[calc(var(--spacing-xl)_+_var(--spacing-s))]">
          <Section title="Appearance" hint="Light or dark, and which built-in theme (colour + shape) the whole app wears.">
            ...
            <div className="flex flex-col gap-s">
              ...
              <div role="group" aria-label="Theme mode" className="inline-flex w-fit gap-xs">
                {THEME_MODES.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => setMode(m)}
                      aria-pressed={active}
                      className={`flex items-center gap-s rounded-interactive px-m py-xs text-xs ring-1 ring-inset transition-colors ${
            ...
            <div className="flex flex-col gap-s">
              ...
              <div role="group" aria-label="Theme" className="flex flex-wrap gap-s">
                {THEME_NAMES.map((t) => {
                  const active = t === name
                  return (
                    <button
                      ...
                      onClick={() => setName(t)}
                      aria-pressed={active}
                      className={`flex items-center gap-s rounded-interactive px-m py-s text-xs ring-1 ring-inset transition-colors ${
                        active ? 'bg-active font-medium text-texthard ring-accent' : 'bg-foreground text-textsoft ring-border hover:bg-rollover'
                      }`}
                    >
          ...
          <Section title="Feedback" hint="Something broken or missing? Open a bug on GitHub.">
            <a
              ...
              className="inline-flex w-fit items-center gap-s rounded-interactive px-m py-s text-xs text-texthard ring-1 ring-inset ring-border transition-colors hover:bg-rollover"
            >
