...
The knowledge base reads a committed `graphify-out/graph.json`, generated by [Graphify](https://graphify.net/) (a separate open-source project, MIT — `pip install graphifyy`, Python ≥ 3.10). You only need to regenerate it (`/graphify . --update`) when a change moves enough code or docs around to stale the graph — `npx roadmapped kb doctor` tells you.
