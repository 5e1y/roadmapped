...
- **Open the ticket in `main` BEFORE the worktree (#241)**: Roadmapped's backlog lives on `main`; a worktree's `docs/tasks/` changes are INVISIBLE to your main dashboard/CLI/MCP until you merge. So the flow is: `quick`/`add` + `start` the ticket in `main` FIRST, then dispatch the worktree agent with that id. The agent works the CODE in isolation and does NOT create or `done` tickets there (they'd be lost until merge, and you can't see progress); YOU `done` the ticket in `main` after integrating its code. Never let the source of truth split across an unmerged worktree — the ticket is the anchor that survives the merge.
