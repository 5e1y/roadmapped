...
1. **Bootstrap, then materialize — two commands, in order.** Roadmapped is published on npm AND installable straight from GitHub (`github:5e1y/roadmapped`) — GitHub stays the single source of truth, npm mirrors it automatically. Note the repo may already have Roadmapped as a Claude Code plugin (`/plugin marketplace add 5e1y/roadmapped` + `/plugin install roadmapped@roadmapped`); in that case the skill is already loaded, but the two commands below still apply to wire the repo itself. So:
   - `npx roadmapped init` — primary. Pin/fallback without the npm registry: `npx --yes github:5e1y/roadmapped init` (runs the CLI straight from GitHub). Lays down ALL the plumbing in one move, idempotent: `roadmapped.config.json`, the `docs/tasks/` skeleton (`_meta.yaml` nextId: 1 + the 9 canonical types with their `_section.yaml`, `baseHeat` seeded from the code defaults), the skill in `.claude/skills/roadmapped/`, the MCP entry in `.mcp.json`, a `SessionStart` hook in `.claude/settings.json` (runs `sitrep` when each session opens — the state of the world is injected upfront, #122), the git guard hook (chained onto an existing pre-commit, never overwritten), and it adds `roadmapped` as a devDependency **sourced from GitHub**. It NEVER touches an already-populated `docs/tasks/` or an existing config.
   - `npm install` — materializes `node_modules/roadmapped/` from that devDependency. REQUIRED: the guard hook, the `SessionStart` hook and the MCP entry all resolve through `node_modules/roadmapped/`, AND it's what makes every subsequent bare `npx roadmapped <cmd>` resolve locally instead of hitting the network. Then tell the user to **restart the Claude session** so the `SessionStart` hook + MCP server load. Until `npm install` has run, `npx roadmapped <cmd>` resolves from the npm registry (or pin to GitHub with `npx github:5e1y/roadmapped <cmd>`).
...
4. Create tasks **via the CLI only** (`add --type <type> ...`, `--type` is the exact folder slug, e.g. `02-feature`), in dependency order (a `--depends-on` can only cite an already-created id). Set `--refs`, `--tags`, `--depends-on`, and `--heat` (only when a deliberate priority signal is warranted) right at creation. `--source user` for what comes from the user's own writing, `ai` for what you infer.
5. Apply the agreed fate for the old files.
6. Final `validate` + `npx roadmapped roadmap` and `list` to show the result to the user.
7. **Knowledge base (Graphify — installed by default)**: `init` ends by installing Graphify, no prompt — it logs what it does and what it weighs (~30 MB one-time, up to ~85 MB on a machine with neither Python nor uv; opt-out `roadmapped init --no-kb`; offline or a failed install never blocks the init — `roadmapped upgrade` retries). The graph is the agent's navigation layer for the repo: up to ~70% fewer **exploration** tokens once it exists. Generating it is an agent act and the ONE step that needs consent — `/graphify .` runs sub-agents over the docs and **costs tokens** — so ask the user once, and on agreement run it once (later refreshes: `/graphify . --update`, code-only, no LLM cost). Never generate without that consent.
