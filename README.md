...
| **Knowledge base** | A toggle in Docs: a live graph of your repo (code + docs) with the tickets wired in via their `refs` — nothing to fill in. Your agent queries it too (`kb_*` MCP tools, `npx roadmapped kb`) to find the right files before exploring blind. Rendered from `graphify-out/graph.json`, built by [Graphify](https://graphify.net/) (open source, MIT). Optional — Python ≥ 3.10 only if you want to generate the graph; Roadmapped stays Node. |
...
The knowledge base graph is generated by [Graphify](https://graphify.net/) — a separate
