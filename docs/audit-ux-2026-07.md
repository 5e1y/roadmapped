...
- [x] **[important]** [panel] Le textarea Détail (min-h 120px) rend illisible un detail long — lecture au lasso dans une lucarne → *#2* (mode lecture acté par la spec approuvée). — fait (vérifié #28) : markdown rendu au repos, clic → textarea auto-grow À TAILLE IDENTIQUE (hauteur mesurée sur le rendu), aucun rétrécissement.
...
- [x] **[nice]** [panel] Chemin du fichier YAML brut sur 2 lignes en tête de panneau — reléguer en pied de panneau → *#2*. — fait (vérifié #28) : chemin mono en pied (mt-auto, filet haut) dans TaskPanel ET SectionPanel.
...
- [x] **[important]** Les erreurs de validation s'affichent en haut du panneau, hors écran quand on édite un champ du bas → *#2* — fait (vérifié #28) : erreurs suivies PAR champ et rendues SOUS la zone fautive (FieldError, ui.tsx), TaskPanel et SectionPanel ; le bandeau haut ne sert plus qu'aux actions de cycle de vie.
...
- [x] **[important]** Les dépendances d'une tâche archivée sont totalement invisibles (info cachée) → *#2* — fait (vérifié #28) : les listes Dépend de/Liens/Références se rendent aussi en mode archivé (lignes RelationRow lecture seule, ✕ masqué), badge d'état des deps compris.
